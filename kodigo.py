...
class FingerprintEnrollment:
    ...
    def back_to_attendance(self):
        """Navigate back to the AttendanceApp from FingerprintEnrollment."""
        self.hide()  # Hide the current FingerprintEnrollment frame
        self.attendance_app.show()  # Show the AttendanceApp frame
...
class AttendanceApp:
    ...
    def start_fingerprint_scanning(self):
        ...
        if not self.finger:  # Check if the fingerprint sensor is already initialized
            self.finger = self.initialize_serial()  # Reinitialize the fingerprint sensor
