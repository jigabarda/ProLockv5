...
class AttendanceApp:
    def __init__(self, root):
        ...
        # Initialize the text-to-speech engine, set fonts, styles, main frame, and other UI elements
    ...
    def record_time_in_fingerprint(self, fingerprint_id, user_name, role_id="2"):
        try:
            ...
            self.update_result("Door unlocked!")
        except requests.RequestException as e:
    ...
    def record_time_out_fingerprint(self, fingerprint_id):
        try:
            ...
            self.update_result("Door locked!")
        except requests.RequestException as e:
    ...
    def auto_scan_fingerprint(self):
        ...
        while self.running:
            ...
            self.update_result("Templating fingerprint...")
            if self.finger.image_2_tz(1) != adafruit_fingerprint.OK:
                self.update_result("Failed to template the fingerprint image.")
                failed_attempts += 1
            ...
            self.update_result("Searching for fingerprint match...")
            ...
            self.update_result(f"Fingerprint matched with ID: {self.finger.finger_id}, confidence: {self.finger.confidence}")

            # Fetch user details using API
            name = self.get_user_details(self.finger.finger_id)

            if name:
                self.update_result(f"Fingerprint belongs to {name}. Checking access schedule...")
